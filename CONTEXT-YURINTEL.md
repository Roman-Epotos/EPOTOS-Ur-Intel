...
Хостинг: Vercel (с 03.07.2026 — Pro план!)
...
ВАЖНО: После перехода на Vercel Pro (03.07.2026) — сделать Redeploy проекта
...
✅ Ссылка из уведомлений Б24 → карточка документа:
...
✅ Ссылка на документ в приветственном сообщении чата согласования:
   createBitrixChat принимает contract_id → формирует [URL=...]...[/URL] в первом сообщении
✅ app/page.tsx переведён в 'use client' для поддержки useEffect/useRouter
✅ Оптимизация главной страницы:
   /api/my-documents грузится один раз в app/page.tsx
   Данные передаются в MyDocuments и PersonalStats через пропсы
   MyDocsData экспортируется как named interface из MyDocuments.tsx
   PersonalStats делает только один запрос /api/contracts-list
...
⚠️ Таймауты загрузки при 50-60 одновременных пользователях — решается
   переходом на Vercel Pro с 03.07.2026 (Fluid Active CPU, Edge CPU на 100%)


=============================================================
8. ГДЕ ОСТАНОВИЛИСЬ (01.07.2026)
=============================================================
Сделано в текущую сессию:
  ✅ Оптимизация главной страницы — устранён дублирующийся запрос /api/my-documents
     app/page.tsx грузит данные → передаёт в MyDocuments(data, loading, onReload)
     и PersonalStats(myDocsData) через пропсы
  ✅ Ссылка на документ в приветственном сообщении чата согласования Б24
     createBitrixChat в notify.ts теперь принимает contract_id (опционально)
     Формат: [URL=https://.../?contract_id=UUID]номер — название[/URL]
     Вызов в app/api/approvals/route.ts передаёт contract_id
...
Напоминание: 03.07.2026 — переход на Vercel Pro → сделать Redeploy после апгрейда!
...
Оптимизация главной страницы:
  ...
  MyDocsData экспортируется как named interface (export interface MyDocsData)
