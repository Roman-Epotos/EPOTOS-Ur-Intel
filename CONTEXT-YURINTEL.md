...
counterparties — УНИКАЛЬНЫЙ КЛЮЧ: (inn, kpp)!
  ...
  НОВОЕ: inn_verified_at (timestamptz) — дата подтверждения ИНН физлица
...
Сделано в текущую сессию:
  ✅ Проверка ИНН физлица по алгоритму ФНС (контрольная сумма, фронт)
     Колонка inn_verified_at добавлена в counterparties
  ✅ Ссылка из чата Б24 → карточка документа (contract_id в URL)
     app/page.tsx — 'use client', useEffect читает contract_id → редирект
  ✅ Кнопка «👁️ Просмотр» для всех форматов в «Подписанных экземплярах»
     (было только для PDF, теперь для любого файла через proxyUrl)

Следующие задачи (по приоритету):
  1. ⬜ Drag & drop загрузка подписанного файла
  2. ⬜ Шаблоны Эпотос-К
  3. ⬜ ЭПОТОС-ПЕРСОНАЛ
