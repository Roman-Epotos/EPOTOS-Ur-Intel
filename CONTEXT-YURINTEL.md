...
✅ Проверка надёжности контрагентов (заменила СБИС интеграцию):
...
✅ Документы контрагента в карточке договора (вкладка Реквизиты/Чат):
  Блок под реквизитами: список по категориям + кнопка загрузки + ссылка на карточку
  Загрузка файлов прямо из карточки договора (без перехода в реестр)
  Кнопка «👁 Открыть» — открывает файл в новом окне
✅ Прокси файлов Supabase Storage через Vercel (доступ из России):
  API: GET /api/file-proxy?url=... — проксирует файлы с supabase.co
  Утилита: app/utils/proxyUrl.ts — proxyUrl(url) оборачивает все ссылки
  Применено в: ContractTabs.tsx, counterparties/[id]/page.tsx, admin/page.tsx
  Транслитерация имён файлов при загрузке (кириллица → латиница, пробелы → _)
  Поддержка файлов до ~10 МБ (maxDuration=60)
...
⚠️ Лимиты загрузки больших PDF во вкладке Документы и подписанных документов
    Статус: требует проверки


=============================================================
9. ГДЕ ОСТАНОВИЛИСЬ (11.06.2026 сессия 2)
=============================================================
Сделано в текущую сессию:
  ✅ Документы контрагента в карточке договора
  ✅ Прокси файлов через Vercel (доступ из России)
  ✅ Транслитерация имён файлов при загрузке
  ✅ Увеличен лимит размера загружаемых файлов

Следующие задачи (по приоритету):
  1. Проверка лимитов загрузки во вкладке Документы и подписанных документов
...
🔄 Спринт 4 (активен):
  ...
  ⬜ Проверка лимитов загрузки больших файлов
...
Файлы контрагента:
  ...
  API: /api/counterparties/[id]/documents (GET/POST/DELETE)
  Путь в Storage: {counterparty_id}/{category}/{timestamp}_{safeName}
  safeName: транслитерация кириллицы + пробелы → _ + только латиница/цифры
...
Прокси файлов Supabase:
  ...
  Применять везде где есть href/src на файлы из Supabase Storage
