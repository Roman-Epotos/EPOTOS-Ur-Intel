...
export default function ExecutionControl({
  ...
  const loadItems = async () => {
    setLoading(true)
    try {
      const [itemsRes, archiveRes] = await Promise.all([
        fetch(`${baseUrl}/api/contract-checklist?contract_id=${contractId}`),
        fetch(`${baseUrl}/api/contract-checklist?contract_id=${contractId}&check_archive=true`),
      ])
      const itemsData = await itemsRes.json()
      const archiveData = await archiveRes.json()
      setItems(itemsData.items ?? [])
      setHasArchive(archiveData.has_archive ?? false)
    } finally {
      setLoading(false)
    }
  }
  ...
  const restoreArchive = async () => {
    if (!confirm('Восстановить предыдущую версию чек-листа?\n\nТекущая версия будет сохранена как предыдущая.')) return
    setGenerating(true)
    try {
      const res = await fetch(`${baseUrl}/api/contract-checklist`, {
        method: 'POST',
    ...
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          {canGenerate && hasArchive && items.length > 0 && (
            <button onClick={restoreArchive} disabled={generating}
              className="text-xs font-medium border border-gray-200 text-gray-600 px-3 py-2 rounded-lg hover:bg-gray-50 disabled:opacity-50 flex items-center gap-1.5">
              ↩️ Предыдущая версия
            </button>
          )}
          {canGenerate && (
