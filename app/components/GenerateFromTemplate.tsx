...
const EXTRA_FIELDS: Record<string, { key: string; label: string; placeholder?: string; type?: string }[]> = {
  ...
  'персданные_эк': [
    ...
    { key: 'passport_series', label: 'Серия паспорта', placeholder: '4510' },
    { key: 'passport_number', label: 'Номер паспорта', placeholder: '123456' },
    { key: 'passport_issued_date', label: 'Дата выдачи паспорта', placeholder: '15 декабря 2020 г.' },
    { key: 'passport_issued_by', label: 'Кем выдан', placeholder: 'ОУФМС России' },
    { key: 'subject_address', label: 'Адрес регистрации', placeholder: 'г. Москва, ул. ...' },
    { key: 'pd_consent_date', label: 'Согласие действует до', placeholder: '31.12.2035', type: 'date' },
  ],
  'pd_consent': [
    { key: 'subject_full_name', label: 'ФИО физлица', placeholder: 'Иванов Иван Иванович' },
    { key: 'subject_birth_year', label: 'Дата рождения (дд.мм.гггг)', placeholder: '01.01.1985' },
...
export default function GenerateFromTemplate({ contract, onUploaded }: GenerateFromTemplateProps) {
  ...
  const [showCpSuggestions, setShowCpSuggestions] = useState(false)
  ...
  const searchCp = async (q: string) => {
    if (q.length < 2) { setCpSuggestions([]); setCpNotFound(false); return }
    setCpSearchLoading(true)
    setCpNotFound(false)
    try {
      const res = await fetch(`${baseUrl}/api/counterparties?search=${encodeURIComponent(q)}&limit=10`)
      const data = await res.json()
      const list = data.counterparties ?? []
      setCpSuggestions(list)
      setShowCpSuggestions(true)
      setCpNotFound(list.length === 0)
    } finally {
      setCpSearchLoading(false)
    }
  }

  const searchCpByInn = async () => {
    if (!addCpInn.trim()) return
    setAddCpLoading(true)
    setAddCpError('')
    setAddCpResult(null)
    try {
      const res = await fetch(`${baseUrl}/api/counterparties/check-inn?inn=${addCpInn.trim()}`)
      const data = await res.json()
      if (data.found) { setAddCpResult(data.data) }
      else { setAddCpError(data.message ?? 'Организация не найдена в DaData') }
    } catch { setAddCpError('Ошибка соединения') }
    finally { setAddCpLoading(false) }
  }
  ...
  const saveCp = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setCpSearch(data.counterparty.short_name ?? data.counterparty.full_name)
        setShowAddCpModal(false)
        setAddCpInn('')
        setAddCpResult(null)
        setCpNotFound(false)
        setShowCpSuggestions(false)
      } else {
        setAddCpError(data.error ?? 'Ошибка сохранения')
      }
    } finally { setAddCpSaving(false) }
  }
      ...
      <div className="space-y-5">
        ...
        {selectedTemplate.type !== 'персданные' && <div className="space-y-2">
          ...
          <div className="flex gap-2 mb-1">
            <button type="button" onClick={() => { setCounterpartyMode('select'); setSelectedCounterparty(null); setCpSearch('') }}
            ...
            <button type="button" onClick={() => { setCounterpartyMode('empty'); setSelectedCounterparty(null); setCpSearch('') }}
            ...
            <div className="relative">
              {selectedCounterparty ? (
                <div className="flex items-center justify-between bg-green-50 border border-green-200 rounded-lg px-3 py-2">
                  <div>
                    <p className="text-sm font-medium text-green-900">{selectedCounterparty.short_name ?? selectedCounterparty.full_name}</p>
                    <p className="text-xs text-green-600">ИНН: {(selectedCounterparty as {inn?: string}).inn ?? '—'}</p>
                  </div>
                  <button type="button" onClick={() => { setSelectedCounterparty(null); setCpSearch(''); setCpNotFound(false) }}
                    className="text-xs text-gray-400 hover:text-gray-700 ml-2">✕</button>
                </div>
              ) : (
                <>
                  <input
                    type="text"
                    value={cpSearch}
                    onChange={e => { setCpSearch(e.target.value); searchCp(e.target.value) }}
                    onBlur={() => setTimeout(() => setShowCpSuggestions(false), 200)}
                    placeholder="Начните вводить название или ИНН..."
                    className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-gray-400"
                  />
                  {cpSearchLoading && <span className="absolute right-3 top-2.5 text-gray-400 text-xs">...</span>}
                  {showCpSuggestions && cpSuggestions.length > 0 && (
                    <div className="absolute z-10 w-full bg-white border border-gray-200 rounded-lg shadow-lg mt-1 max-h-48 overflow-y-auto">
                      {cpSuggestions.map(c => (
                        <button key={c.id} type="button"
                          onMouseDown={() => { setSelectedCounterparty(c); setCpSearch(c.short_name ?? c.full_name); setShowCpSuggestions(false); setCpNotFound(false) }}
                          className="w-full text-left px-3 py-2 hover:bg-gray-50 border-b border-gray-100 last:border-0">
                          <p className="text-sm font-medium text-gray-900">{c.short_name ?? c.full_name}</p>
                          <p className="text-xs text-gray-400">ИНН: {(c as {inn?: string}).inn ?? '—'}</p>
                        </button>
                      ))}
                    </div>
                  )}
                  {cpNotFound && cpSearch.length >= 2 && (
                    <div className="absolute z-10 w-full bg-white border border-orange-200 rounded-lg shadow-lg mt-1 px-3 py-3">
                      ...
                      <p className="text-xs text-gray-500 mb-2">Сначала добавьте контрагента в базу.</p>
                      <button type="button"
                        onMouseDown={() => { setShowAddCpModal(true); setShowCpSuggestions(false); setCpNotFound(false) }}
              ...
              {showAddCpModal && (
                <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 px-4">
                  <div className="bg-white rounded-xl p-6 max-w-md w-full shadow-xl">
                    ...
                    {!addCpResult ? (
                      <>
                        <p className="text-sm text-gray-500 mb-3">Введите ИНН — данные будут загружены из DaData.</p>
                        <div className="flex gap-2 mb-3">
                          <input value={addCpInn} onChange={e => setAddCpInn(e.target.value)}
                            onKeyDown={e => e.key === 'Enter' && searchCpByInn()}
                            placeholder="ИНН (10 или 12 цифр)"
                            className="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900" />
                          <button type="button" onClick={searchCpByInn} disabled={addCpLoading}
                      ...
                      <>
                        ...
                        <div className="flex gap-2">
                          <button type="button" onClick={saveCp} disabled={addCpSaving}
                    ...
                    <button type="button" onClick={() => { setShowAddCpModal(false); setAddCpInn(''); setAddCpResult(null); setAddCpError('') }}
                      className="w-full mt-3 text-sm text-gray-400 hover:text-gray-600">Отмена</button>
                  </div>
                </div>
              )}
            </div>
