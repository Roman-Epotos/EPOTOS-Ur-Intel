...
ВАЖНЫЕ ПРАВИЛА:
1. ИГНОРИРУЙ полностью:
   - подписи, печати, штампы ЭДО, QR-коды, отметки об электронном подписании
   - строки с подчёркиваниями для подписей: "________________"
   - пустые поля в реквизитах (факс, банковские реквизиты контрагента)
   - колонтитулы, нумерацию страниц, технические метаданные PDF
   - разрывы страниц в PDF которые могут склеивать текст из разных пунктов
   - незаполненные поля вида "____________", "«____»", "№ ______"
2. Сравнивай ТОЛЬКО смысловое содержание: условия договора, права и обязанности сторон, суммы, сроки
3. Различия в форматировании, пробелах, переносах строк — НЕ расхождения
4. Если смысловое содержание документов идентично — match: true, discrepancies: []
5. Расхождение — это только реальное изменение условий: другая сумма, другой срок, изменённое обязательство, добавленный или удалённый пункт
6. НЕ считай расхождением: разный порядок слов в одном предложении, разные пустые поля, текст из соседнего пункта попавший из-за разрыва страницы PDF
