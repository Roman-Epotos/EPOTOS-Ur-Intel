...
export default function NewContractPage() {
  ...
  const [counterpartyRisk, setCounterpartyRisk] = useState<'low' | 'medium' | 'high' | null>(null)
  const [counterpartyRiskLoading, setCounterpartyRiskLoading] = useState(false)

  const fetchRisk = async (inn: string, cpId: string, name: string) => {
    if (inn.startsWith('FOREIGN')) return
    setCounterpartyRiskLoading(true)
    setCounterpartyRisk(null)
    try {
      const res = await fetch('https://epotos-ur-intel.vercel.app/api/counterparties/check', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ inn, counterparty_id: cpId, name }),
      })
      const data = await res.json()
      if (data.risk_level) setCounterpartyRisk(data.risk_level)
    } catch { /* тихо игнорируем */ }
    finally { setCounterpartyRiskLoading(false) }
  }
  ...
  const selectCounterparty = (c: {id: string, short_name: string | null, full_name: string, inn: string}) => {
    ...
    setShowCounterpartySuggestions(false)
    fetchRisk(c.inn, c.id, c.short_name ?? c.full_name)
  }
                ...
                <div className="relative">
                  ...
                  {counterpartyId && (
                    <span className="absolute right-3 top-2.5 flex items-center gap-1">
                      ...
                      {counterpartyRiskLoading && <span className="text-gray-400 text-xs">🔍</span>}
                      {!counterpartyRiskLoading && counterpartyRisk === 'low' && <span className="text-xs text-green-600 font-medium">🟢</span>}
                      {!counterpartyRiskLoading && counterpartyRisk === 'medium' && <span className="text-xs text-yellow-600 font-medium">🟡</span>}
                      {!counterpartyRiskLoading && counterpartyRisk === 'high' && <span className="text-xs text-red-600 font-medium">🔴</span>}
                    </span>
                  )}
