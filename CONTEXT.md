...
✅ Кнопка «Отклонить документ» в интерфейсе согласования (с модалом причины)
✅ AI (EpotosGPT): Legal Review, Паспорт, анализ вложений, чат — БЕЗ чек-листа
...
✅ Уведомления Битрикс24 — ПОЛНАЯ СХЕМА (20.05.2026):
  ...
  ВАЖНО: notify.ts нельзя редактировать через PowerShell — только через VS Code!
         Эталонная копия: outputs/notify.ts (сохранить отдельно)
...
⚠️ addUserToBitrixChat при добавлении участника — ещё не подключён в participants/route.ts
...
=============================================================
Пункт A Спринта 2 — личные сообщения и групповой чат — РЕАЛИЗОВАН ✅
Все уведомления от ЭПОТОС Ассистента (ID: 678) ✅
AIAnalysis.tsx — чек-лист убран, компонент восстановлен ✅
npm run build — проверяем перед каждым деплоем ✅

Следующие задачи по Спринту 2:
  🔄 Подключить addUserToBitrixChat в participants/route.ts
     (при добавлении нового участника — добавлять его в чат Б24)
...
🔄 СПРИНТ 2 (июнь 2026) — В РАБОТЕ:
  ...
  🔄 Подключить addUserToBitrixChat при добавлении участника
...
ВАЖНО: PowerShell Set-Content портит кодировку — использовать построчную замену
...
ВАЖНО: notify.ts — только через VS Code (Ctrl+A, удалить, вставить)
        При повреждении — скачать эталон из outputs/notify.ts
ВАЖНО: AIAnalysis.tsx — чек-лист убран! Не добавлять обратно.
        Чек-лист только в ExecutionControl.tsx (вкладка Контроль исполнения)
