...
⚠️ Скачивание файлов у Ершовой (Firefox/Yandex) — CORS в iframe Битрикс24
   Решение: проксировать через /api/onlyoffice/file
...
=============================================================
10. ГДЕ ОСТАНОВИЛИСЬ (04.06.2026)
=============================================================
Сделано в последние сессии:
  ✅ RLS безопасность — все таблицы защищены
  ✅ AI сравнение PDF подписанного документа (Claude через OpenRouter)
  ✅ Legal Review и Паспорт для DOCX — промпты на русском, игнорируют форматирование
  ✅ PDF анализ — понятная ошибка с просьбой конвертировать в DOCX
  ✅ Тип «Спецификация» добавлен
  ✅ Поиск по номеру контрагента (customer_number)
  ✅ Клик по названию документа открывает карточку
  ✅ Карточка контрагента: документы по статусам + поиск по тексту
  ✅ Список согласующих: метки (согласует/ознакамливается) + цвета
  ✅ Inline edit реквизитов + блокировка после «Согласован»

Следующая сессия — план (по приоритетам):
  1. Исправить скачивание файлов у Ершовой (CORS/iframe — проксирование через API)
  2. Контрагенты в Генерации — лимит 10 + поиск по вводу
  3. ЭДО-голосование в согласовании
  4. Шаблоны СПТ, ОС, Эпотос-К (когда будут файлы)
  5. Проверить финансовый дашборд
  6. Импорт старых договоров через Excel — после Эпотос-Core
  7. Мобильная адаптация — после Эпотос-Core
  8. СБИС интеграция
  9. Календарь судебных заседаний (после ТЗ)
