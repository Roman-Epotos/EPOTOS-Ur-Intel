...
✅ AI СРАВНЕНИЕ ПОДПИСАННОГО ДОКУМЕНТА:
  - PDF → base64 → Claude (anthropic/claude-sonnet-4-5) через OpenRouter — ПЛАТНАЯ!
...
✅ СОРТИРОВКА НА ГЛАВНОЙ (Realtime):
  ...
  - Применяется при загрузке и при Realtime обновлении

✅ ЭДО-ГОЛОСОВАНИЕ (частично):
  - Новая роль finance_company (Бухгалтер компании)
  - Специалисты ЭДО: справочник в Настройки → Согласующие → «Специалисты ЭДО»
  - Блок «🖊 Способ подписания» в правой колонке вкладки Согласование
  - Шаг 1: инициатор/юрист/бухгалтер запрашивает разрешение у ГД
    ГД выбирается из approval_settings (stage='director', company=company_prefix)
  - Шаг 2: ГД видит кнопки «Разрешить ЭДО» / «Отклонить»
  - Шаг 3: бухгалтер (finance_gc/finance_company) выбирает метод + специалиста ЭДО
  - Шаг 4: статус документа → на_подписи_в_эдо (если выбрано ЭДО)
  - API: POST /api/approvals/[sessionId]/edo (actions: request, director_decision, set_method)
  ⚠️ НЕ использовать таблицу company_directors для ЭДО — ГД берётся из approval_settings!
  ⚠️ Таблица company_directors создана но пустая — не удалять, пригодится в будущем
...
⚠️ Поиск по customer_number на главной — не реализован
⚠️ Sticky шапка при скролле — не реализована
...
=============================================================
10. ГДЕ ОСТАНОВИЛИСЬ (05.06.2026)
=============================================================
Сделано в эту сессию:
  ✅ Специалисты ЭДО — вкладка в Настройки → Согласующие
  ✅ Роль finance_company добавлена во все нужные места
  ✅ ЭДО-голосование в карточке документа (правая колонка)
  ✅ API /api/approvals/[sessionId]/edo (request, director_decision, set_method)
  ✅ Новый статус на_подписи_в_эдо в БД и contracts
  ✅ ГД для ЭДО берётся из approval_settings (stage=director, company=prefix)

Следующая сессия — план (по приоритетам):
  1. Поиск по customer_number на главной странице
  2. Sticky шапка при скролле (главная, реестр контрагентов и др.)
  3. Шаблоны СПТ, ОС, Эпотос-К (когда будут файлы)
  4. Финансовый дашборд — проверка
  5. Импорт старых договоров через Excel — после Эпотос-Core
  6. Мобильная адаптация — после Эпотос-Core
  7. СБИС интеграция
  8. Календарь судебных заседаний (после ТЗ)
  9. Инструкция пользователя + ЭпотосGPT-Помощник (RAG)
  10. Эпотос-РИД — после Эпотос-Core
...
🔄 Спринт 3:
  ...
  ✅ ЭДО-голосование (частично — блок в карточке работает)
  ✅ Роль finance_company + Специалисты ЭДО в Настройках
  ⬜ Поиск по customer_number
  ⬜ Sticky шапка при скролле
  ⬜ Шаблоны остальных компаний
  ⬜ Финансовый дашборд — проверка

🔴 Спринт 3 финал (ЮрИнтел завершение):
  - Инструкция пользователя (DOCX документ)
  - ЭпотосGPT-Помощник (RAG на pgvector + Gemini)
    Архитектура: help_knowledge + help_chunks (embedding vector(768))
    Поиск: pgvector cosine similarity
    Интерфейс: вкладка «🤖 ИИ-помощник» в /help
    Загрузка базы знаний: Настройки → «📚 База знаний помощника» (DOCX/XLSX/TXT)
    ИИ-помощник — ОТДЕЛЬНЫЙ чат, не в карточке документа
...
ЭДО-голосование:
  ...
  Блок в правой колонке вкладки Согласование, только если !['подписан','на_исполнении']
  API: /api/approvals/[sessionId]/edo — actions: request, director_decision, set_method
  Уведомления через /api/bitrix-notify (POST, не импорт функции)
  Статус на_подписи_в_эдо устанавливается при set_method='edo'
...
Типы шаблонов → EXTRA_FIELDS:
  ...
  асц → 'асц', поставка → 'поставка' / 'поставка_снг', nda → 'nda'
  эдо → 'эдо', персданные → 'персданные'
