...
Дата: 2026-05-22 (финал сессии)
Статус: Спринт 3 в работе — юридический дашборд реализован
...
ВАЖНО: В Битрикс24 имя хранится как "Фамилия Имя" → приветствие: split(' ')[1]
...
gc_manager (1, 246, 504) — Чащина Елена (1), Ершова (246), Кочкин (504)
  ВАЖНО: Чащина Елена (1) — одновременно gc_manager и ГД ООО Техно (ТХ)
  Имеет полный доступ ко всем компаниям (как gc_manager)
...
Карта директор → компания:
  1 (Чащина) → ТХ (но имеет доступ ко всем как gc_manager)
  592 → НПП
  6 → СПТ, ОС
  954 → Э-К
...
✅ СПРИНТ 3 — В РАБОТЕ (22.05.2026):
  ...
  ✅ Рабочий стол /dashboard:
    ...
    Блок «Аналитика» виден: admin + gc_manager + directors (IDs: 30,1148,1,246,504,592,6,954)
    Главная / = список документов (app/page.tsx)

  ✅ Юридический дашборд /dashboard-legal:
    Страница: app/dashboard-legal/page.tsx
    API: app/api/dashboard-legal/route.ts
    Блоки:
      - Документы по статусам (все статусы кроме архив)
      - Просроченные согласования (дедлайн < сегодня)
      - Согласованы но ожидают подписания (статус = согласован)
      - Просроченные пункты чек-листа (без задачи Б24)
      - Динамика создания документов за период (по компаниям)
    Периоды: 30 дней / Квартал (90) / Полгода (180) / Год (365)
    Доступ:
      - admin (30, 1148) — все компании
      - gc_manager (1, 246, 504) — все компании
      - director (592, 6, 954) — только своя компания
    ВАЖНО: фильтрация по компании для директоров — ещё не реализована!
           Нужно добавить параметр company_prefix в API и страницу
  ...
  ✅ Исправления интерфейса:
    ...
    - Фильтр вложений в my-documents/route.ts
    - Запрет загрузки версий после согласования
    - Приветствие по имени split(' ')[1]


=============================================================
9. ИЗВЕСТНЫЕ ПРОБЛЕМЫ / ТЕХНИЧЕСКИЙ ДОЛГ
=============================================================
⚠️ Юр. дашборд: нет фильтрации по компании для директоров — добавить в след. сессии
...
=============================================================
Последнее сделанное: юридический дашборд + блок «Аналитика» на рабочем столе ✅

Первое что делаем в следующей сессии:
  1. Добавить фильтрацию по company_prefix для директоров в dashboard-legal
     (API: передавать company_prefix, страница: определять роль и фильтровать)
  2. Продолжить дашборды или перейти к модулю шаблонов
...
🔄 СПРИНТ 3 (май-июнь 2026) — В РАБОТЕ:
  ✅ Рабочий стол (Personal Feed) + блок Аналитика
  ✅ Реестр контрагентов + DaData
  ✅ Привязка контрагента при создании документа
  🔄 Юридический дашборд (базовая версия — нужна фильтрация по директорам)
  ⬜ Финансовый дашборд
...
- CoreB24Adapter — изоляция Битрикс24 за адаптером (прототип: app/lib/bitrix/tasks.ts)
- Universal Activity Feed / core_activity_feed таблица
- AI Digest на рабочем столе (ежедневная сводка)
- AI-Audit: сохранять промпт в contract_logs при AI-действиях — Спринт 4
- ИНН+КПП как составной уникальный ключ — ЭПОТОС-Core
- RAG метаданные: security_level, expires_at, linked_entities
- RBAC + ABAC (Спринт 4-5)
...
ВАЖНО: Редактировать файлы ТОЛЬКО через VS Code (не PowerShell!)
...
Согласование:
  ...
  Фильтр вложений: category !== 'attachment' в my-documents/route.ts
  Запрет загрузки при статусах: согласован/загружен_частично/подписан/на_исполнении

Юр. дашборд доступ:
  admin + gc_manager → все компании
  director (592, 6, 954) → только своя компания (фильтр ещё не реализован!)
