...
✅ Кнопка «Отклонить документ» в интерфейсе согласования
...
✅ Уведомления Битрикс24 — СХЕМА (19.05.2026):
  ...
  Групповой чат Битрикс24 (im.chat.add):
    ...
    - При согласовании/отклонении → сообщение в чат
...
Не проверено: от чьего имени приходят уведомления после смены вебхука
              на ЭПОТОС Ассистента (ID: 678) — проверить при следующей сессии
...
ВАЖНО: Для правки файлов через PowerShell — построчно (по одной команде)
ВАЖНО: Set-Content портит кодировку — использовать [System.IO.File]::WriteAllText
...
ВАЖНО: notify.ts нельзя редактировать через PowerShell — только через VS Code
        (файл часто портится — хранить эталонную копию в outputs/notify.ts)
