...
Дата: 2026-05-22 (обновлён — исправления интерфейса)
...
ВАЖНО: В Битрикс24 имя хранится как "Фамилия Имя" — для приветствия брать split(' ')[1]
...
=============================================================
8. РЕАЛИЗОВАННЫЙ ФУНКЦИОНАЛ (22.05.2026)
=============================================================
...
✅ СПРИНТ 3 — В РАБОТЕ (22.05.2026):

  ✅ Рабочий стол /dashboard:
    Страница: app/dashboard/page.tsx
    API: app/api/dashboard/route.ts
    Блоки: требуют согласования, дедлайны чек-листа, черновики,
           активные согласования, контрагенты с высоким риском, статистика
    Навигация: Header.tsx — кнопка «🖥️ Рабочий стол»
    Главная / = список документов (app/page.tsx)
    ВАЖНО: приветствие — user?.name?.split(' ')[1] (Фамилия Имя в Б24)

  ✅ Реестр контрагентов:
    Страницы: app/counterparties/page.tsx, app/counterparties/[id]/page.tsx
    API: app/api/counterparties/route.ts, app/api/counterparties/check-inn/route.ts
    DaData: POST .../findById/party, Authorization: Token {DADATA_API_KEY}
    Функции: поиск, добавление по ИНН, редактирование, список договоров, риск

  ✅ Привязка контрагента при создании документа:
    counterparty_id сохраняется в contracts при создании ✅
    Поле с автодополнением из реестра + ввод вручную

  ✅ Исправления интерфейса (22.05.2026):
    - Дедупликация участников согласования по bitrix_user_id в approvals/route.ts
    - Фильтр вложений в my-documents/route.ts (не показывать attachment в Мои документы)
    - Запрет загрузки версий/дополнительных материалов после согласования
      (статусы: согласован, загружен_частично, подписан, на_исполнении)
    - Приветствие по имени (split(' ')[1]) — Фамилия Имя в Битрикс24
...
⚠️ Поля доверенности в реквизитах компаний и контрагентов — запланировано:
   signatory_name, poa_number, poa_date, poa_expires


=============================================================
10. ГДЕ ОСТАНОВИЛИСЬ (22.05.2026)
=============================================================
✅ Рабочий стол — реализован и исправлен
✅ Реестр контрагентов — реализован
✅ Дедупликация участников согласования — исправлено

Следующие задачи Спринта 3:
  ⬜ B. Дашборды (юридический, финансовый, ГД, руководитель ГК)
  ⬜ C. Модуль создания документов из шаблонов ({{field}} + docxtemplater)
...
- AI-Audit: сохранять промпт в contract_logs при AI-действиях — Спринт 4
- ИНН+КПП как уникальный ключ в company_requisites — ЭПОТОС-Core
...
ВАЖНО: На Windows — редактировать файлы ТОЛЬКО через VS Code (не PowerShell!)
...
ВАЖНО: PowerShell НЕЛЬЗЯ использовать для редактирования .ts/.tsx файлов!
...
Согласование:
  Дедупликация участников по bitrix_user_id в approvals/route.ts
  Фильтр вложений в my-documents/route.ts: category !== 'attachment'
  Запрет загрузки версий при статусах: согласован/загружен_частично/подписан/на_исполнении
